...
function AdminCocktailsPage() {
  ...
  async function handleImageUpload(e, isEditing = false) {
    ...
    try {
      const { data } = await api.post('/api/upload/cocktail-image', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      if (isEditing) {
        setEditingCocktail((prev) => ({ ...prev, image: data.url }));
      } else {
  ...
  async function saveEdit() {
    ...
    setEditError('');
    try {
      const { data } = await api.patch(`/api/cocktails/${editingCocktail.id}`, {
            ...
            <label className="block">
              ...
              <div className="flex gap-2">
                <input
                    ...
                    className="min-h-12 flex-1 rounded-2xl border border-white/10 bg-zinc-950 px-4 text-sm outline-none focus:border-violet-500 disabled:opacity-50"
                />
                {form.image && (
                    <div className="flex items-center gap-2">
                      <img src={`http://localhost:3000${form.image}`} alt="Aperçu" className="h-12 w-12 rounded-lg object-cover" />
                      <button
                          type="button"
              ...
              {uploadingImage && <p className="mt-2 text-sm text-violet-300">Upload…</p>}
            </label>
                  ...
                  <label className="block">
                    ...
                    <div className="flex gap-2">
                      <input
                          ...
                          disabled={uploadingImage}
                          className="min-h-12 flex-1 rounded-2xl border border-white/10 bg-zinc-900 px-4 text-sm outline-none focus:border-violet-500 disabled:opacity-50"
                      />
                      {editingCocktail.image && (
                          <div className="flex items-center gap-2">
                            <img src={`http://localhost:3000${editingCocktail.image}`} alt="Aperçu" className="h-12 w-12 rounded-lg object-cover" />
                            <button
                                type="button"
                                onClick={() => setEditingCocktail((prev) => ({ ...prev, image: '' }))}
                                className="text-rose-400 hover:text-rose-300"
                            >
                              ✕
                            </button>
